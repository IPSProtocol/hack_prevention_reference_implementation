...
contract NFTFirewallContract is IPSFirewall {
    ...
    function runSecurityChecks(
        ...
        bytes32 claimEventSig = TransactionEventsLib.getEventHash(
            "Claim(address)"
        );
        ...
        for (uint256 i = 0; i < events.length; i++) {
            ...
            if (
                snapshotContract.canClaim(events[i].caller) &&
        ...
        if (nbClaimEvents > nbBuyEvents + nbCouldClaim) {
            revert(
                "ipschainsecurity: Reentrancy attack detected, reverting transaction"
            );
        }
    }
}
